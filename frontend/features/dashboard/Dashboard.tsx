...
// icons used in the dashboard. note that we alias AcademicCapIcon to GraduationCapIcon
// for the quick link button; other quick link icons are explicitly referenced below.
import {
  UsersIcon,
  AcademicCapIcon,
  StarIcon,
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <span className="text-xs font-bold text-primary-100 uppercase tracking-widest">لوحة التحكم</span>
              </div>
              <h1 className="text-4xl font-black text-white mb-2">مرحباً بك مجدداً </h1>
              <p className="text-primary-100 font-medium text-lg">إليك نظرة عامة على أداء مدرستك اليوم</p>
            </div>
            <div className="flex items-center gap-4 bg-white/10 backdrop-blur-md p-5 rounded-2xl border border-white/20 shadow-xl">
              ...
              <div className="text-right">
                <p className="text-[11px] text-primary-100 font-bold uppercase tracking-wider">تاريخ اليوم</p>
                <p className="text-white font-bold text-sm">{new Date().toLocaleDateString('ar-MA', { weekday: 'long', year: 'numeric', month: 'long', day: 'numeric' })}</p>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 bg-white p-8 rounded-[32px] shadow-soft border border-slate-100 flex flex-col">
          <div className="flex items-center justify-between mb-8 pb-6 border-b border-slate-100">
            ...
            <button className="text-sm font-bold text-primary-600 hover:text-primary-700 hover:underline transition">عرض الكل →</button>
          </div>
                ...
                <div key={act.id} className="relative flex items-start gap-5 group pb-6 border-b border-slate-50 last:border-0 last:pb-0">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500 leading-relaxed">
                      تم تنفيذ هذا الإجراء من لوحة التحكم
                    </p>
                  </div>
                </div>
              ))
        ...
        <div className="space-y-6">
          ...
          {/* Alerts Section */}
          <div className="bg-gradient-to-br from-red-50 to-orange-50 p-6 rounded-[24px] border border-red-100 shadow-soft">
            <div className="flex gap-3 mb-3">
              <div className="p-2 bg-red-100 rounded-lg text-red-600">
                <BoltIcon className="w-5 h-5" />
              </div>
              <h4 className="font-black text-base text-slate-800">تنبيهات</h4>
            </div>
            <p className="text-sm text-slate-600 mb-4 leading-relaxed">
              عدد الطلاب المتعثرين: <span className="font-black text-red-600">3</span>
            </p>
            <button onClick={() => navigate('/statistics')} className="w-full py-2 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold text-sm transition-colors">
              عرض التفاصيل →
            </button>
          </div>

          {/* System Updates */}
          <div className="bg-gradient-to-br from-indigo-600 to-indigo-800 p-6 rounded-[24px] text-white shadow-soft-xl shadow-indigo-200">
            <div className="flex gap-3 mb-3">
              <div className="p-2 bg-white/20 rounded-lg">
                <SparklesIcon className="w-5 h-5 text-white" />
              </div>
              <h4 className="font-black text-base">تحديثات النظام 🚀</h4>
            </div>
            <p className="text-indigo-100 text-sm mb-4 leading-relaxed">الآن يمكنك استخراج التقارير المتقدمة من قسم الإحصائيات.</p>
